...
function currentPage(normalizedPath) {
...
<details class="ls-native-menu" data-ls-mobile-public-polish="v3">
  ...
  <div class="ls-native-panel" role="navigation" aria-label="เมนูมือถือ">
    ...
    <div class="ls-native-note">Mobile Public Polish V3 · Tools Router V1 · no fake runtime</div>
  </div>
</details>`;
...
function routeToolsCard(html, title, href, label) {
  ...
  const re = new RegExp('(<a class="card"\\s+)href="#"([^>]*>[\\s\\S]*?<h3>' + safeTitle + '<\\/h3>)', 'i');
...
.ls-coming-soon-toast{position:fixed;left:16px;right:16px;bottom:88px;z-index:900;display:none;max-width:520px;margin:auto;padding:14px 16px;border-radius:14px;border:1px solid rgba(247,201,106,.28);background:rgba(14,12,8,.96);box-shadow:0 16px 44px rgba(0,0,0,.45);color:var(--fg2,var(--fg-secondary,#a2a7b0));font-family:var(--fd,var(--font-body,"Inter","Noto Sans Thai",system-ui,sans-serif));line-height:1.55}.ls-coming-soon-toast.show{display:block}.ls-coming-soon-toast b{display:block;color:#f7c96a;font-family:var(--fm,var(--font-mono,"IBM Plex Mono",monospace));font-size:.72rem;letter-spacing:.12em;margin-bottom:4px}[data-ls-tool-route]{border-color:rgba(99,179,255,.22)!important}[data-ls-tool-route] .go{color:var(--acc,#63b3ff)!important}
  ...
  const script = `
<script data-ls-tools-router="v1">
(function(){
  var toast=document.getElementById('ls-coming-soon-toast');
  document.addEventListener('click',function(e){
    var a=e.target.closest('a.card[href="#"]');
    if(!a)return;
    e.preventDefault();
  ...
  html = html.replace(/<\/head>/i, style + '\n</head>');
  html = html.replace(/<\/body>/i, toast + '\n' + script + '\n</body>');
  return html;
}
...
(function(){
  ...
  var hints={writer:'เขียนโพสต์ / landing copy / email / caption ที่ต้องการ',image:'อธิบายภาพที่ต้องการสร้าง พร้อมสไตล์และขนาด',research:'ใส่หัวข้อที่ต้องการค้นคว้าและระดับความลึก',code:'วางโค้ดหรืออธิบาย bug ที่ต้องการแก้'};
  if(prompt)prompt.placeholder=label+' — '+(hints[tool]||'พิมพ์คำสั่งที่ต้องการ');
  if(state)state.textContent='tool: '+tool+' · runtime: not wired';
  if(provider)provider.setAttribute('aria-label','Provider target for '+label);
  if(log){
    var msg=document.createElement('div');
    msg.className='msg bot';
    msg.innerHTML='<div class="role">TOOL ROUTER</div><div></div>';
    msg.lastChild.textContent='เปิดจาก Tools → '+label+' แล้ว แต่ Runtime ยังไม่ wired และจะไม่สร้างคำตอบปลอม';
    log.appendChild(msg);
  }
})();
</script>`;
  ...
  html = html.replace(/(<div class="notice[\s\S]*?<\/div>)/i, '$1\n' + badge);
...
export default {
  async fetch(request, env) {
    ...
    if (page === 'chat.html') html = applyChatToolContext(html, url.searchParams.get('tool'));
    html = mobilePolish(html, normalizedPath);

    return new Response(html, { status: response.status, headers: htmlHeaders(response, 'tools-router-v1') });
  }
};